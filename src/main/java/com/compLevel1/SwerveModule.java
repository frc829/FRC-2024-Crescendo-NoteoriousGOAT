...
public class SwerveModule {
    ...
    public static final Function<Motor, Function<Motor, Function<Double, Function<Double, Function<Measure<Distance>, Function<Integer, Function<String, SwerveModule>>>>>>> create = (
            steerMotor) -> (
                    wheelMotor) -> (steerGearing) -> (wheelGearing) -> (wheelRadius) -> (angleSensorId) -> (canbus) -> {
                        MutableMeasure<Angle> angle = MutableMeasure.zero(Rotations);
                        MutableMeasure<Angle> absoluteAngle = MutableMeasure.zero(Rotations);
                        MutableMeasure<Distance> wheelPosition = MutableMeasure.zero(Meters);
                        MutableMeasure<Velocity<Distance>> wheelVelocity = MutableMeasure.zero(MetersPerSecond);

                        Sensor<Angle> angleSensor = Sensor.CTRE.createAngleSensor
                                .apply(Sensor.CTRE.createCancoder.apply(angleSensorId).apply(canbus)
                                        .apply(absoluteAngle));

                        MutableMeasure<Angle> steerSetpoint = MutableMeasure.zero(Radians);
                        ...
                        MutableMeasure<Angle> resetSetpoint = MutableMeasure.zero(Rotations);
                        Runnable resetSteerEncoderFromAbsolute = () -> steerMotor.setRelativeEncoderAngle
                        ...
                        Runnable update = () -> {
                            ...
                            absoluteAngle.mut_setMagnitude(steerMotor.angle.in(Rotations) / steerGearing);
                            angleSensor.update.run();
                            resetSetpoint.mut_setMagnitude(angleSensor.measure.in(Rotations) * steerGearing);
                            angle.mut_setMagnitude(steerMotor.angle.in(Rotations) / steerGearing);
                            wheelPosition.mut_setMagnitude(
                                    wheelMotor.angle.in(Radians) * wheelRadius.in(Meters) / wheelGearing);
                            wheelVelocity.mut_setMagnitude(wheelMotor.angularVelocity.in(RadiansPerSecond)
                                    * wheelRadius.in(Meters) / wheelGearing);
